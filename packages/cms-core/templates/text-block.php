<?php
use Easeo\Cms\Content\ContentRepository;
/**
 * Template: Text block section
 * Expects: $data array with titel, tekst, achtergrond (light/white/dark)
 */
$titel = ContentRepository::escape($data['titel'] ?? '');
$tekst = $data['tekst'] ?? '';
$bg = ($data['achtergrond'] ?? 'white') === 'light' ? 'bg-light' : (($data['achtergrond'] ?? '') === 'dark' ? 'bg-dark text-white' : 'bg-white');
$centered = !empty($data['gecentreerd']);
?>
<section class="py-16 <?php 
echo $bg;
?>">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 <?php 
echo $centered ? 'text-center' : '';
?>">
        <?php 
if ($titel) {
    ?>
        <h2 class="text-3xl font-display font-bold mb-4"><?php 
    echo $titel;
    ?></h2>
        <?php 
}
?>
        <?php 
if ($tekst) {
    ?>
        <div class="content-area text-lg leading-relaxed <?php 
    echo $bg === 'bg-dark text-white' ? 'text-gray-300' : 'text-muted';
    ?>">
            <?php 
    echo nl2br(ContentRepository::escape($tekst));
    ?>
        </div>
        <?php 
}
?>
    </div>
</section>
<?php 
