...
function save_posts(array $posts) : bool
{
    $data = get_posts_data();
    $data['posts'] = $posts;
    return ContentRepository::saveJson('posts.json', $data);
}
function get_published_posts() : array
{
    $posts = get_posts();
    $now = date('Y-m-d H:i:s');
    return array_filter($posts, fn($p) => ($p['status'] ?? 'concept') === 'gepubliceerd' && ($p['datum'] ?? '') <= $now);
}
...
function create_post(array $data) : array
{
    ...
    $post = ['id' => substr(md5(uniqid(mt_rand(), true)), 0, 12), 'titel' => trim($data['titel'] ?? ''), 'slug' => $slug, 'samenvatting' => trim($data['samenvatting'] ?? ''), 'inhoud' => $data['inhoud'] ?? '', 'afbeelding' => $data['afbeelding'] ?? '', 'categorie' => trim($data['categorie'] ?? ''), 'tags' => $data['tags'] ?? '', 'auteur' => $data['auteur'] ?? $_SESSION['easeo_admin']['naam'] ?? '', 'status' => in_array($data['status'] ?? '', ['gepubliceerd', 'concept']) ? $data['status'] : 'concept', 'datum' => $data['datum'] ?? date('Y-m-d H:i:s'), 'bijgewerkt' => date('Y-m-d H:i:s'), 'meta_title' => trim($data['meta_title'] ?? ''), 'meta_description' => trim($data['meta_description'] ?? '')];
    $posts[] = $post;
    save_posts($posts);
    return $post;
}
...
function paginate_posts(array $posts, int $page = 1, int $perPage = 9) : array
{
    ...
    return ['posts' => array_slice($posts, $offset, $perPage), 'page' => $page, 'total_pages' => $totalPages, 'total' => $total];
}
