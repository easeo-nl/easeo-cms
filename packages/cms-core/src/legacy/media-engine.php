...
if (!defined('MEDIA_THUMB_DIR')) {
    define('MEDIA_THUMB_DIR', EASEO_ROOT . '/images/thumbs');
}
if (!defined('MEDIA_MAX_SIZE')) {
    define('MEDIA_MAX_SIZE', 10 * 1024 * 1024);
}
// 10MB
...
if (!defined('MEDIA_THUMB_WIDTH')) {
    define('MEDIA_THUMB_WIDTH', 300);
}
if (!defined('MEDIA_THUMB_HEIGHT')) {
    define('MEDIA_THUMB_HEIGHT', 300);
}
if (!defined('MEDIA_MAX_WIDTH')) {
...
{
    return ContentRepository::saveJson('media.json', ['files' => $files]);
}
function upload_media(array $file) : array
{
    ...
    $media = get_media();
    $entry = ['id' => substr(md5(uniqid(mt_rand(), true)), 0, 12), 'bestandsnaam' => $filename, 'origineel' => $file['name'], 'type' => $mimeType, 'grootte' => filesize($uploadPath), 'url' => '/images/uploads/' . $filename, 'thumb' => file_exists($thumbPath) ? '/images/thumbs/' . $filename : '/images/uploads/' . $filename, 'datum' => date('Y-m-d H:i:s')];
    $media[] = $entry;
    save_media($media);
