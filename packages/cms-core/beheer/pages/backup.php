...
if (isset($_GET['action']) && $_GET['action'] === 'download') {
    ...
    if ($zip->open($zipFile, ZipArchive::OVERWRITE) !== true) {
        $_SESSION['flash_error'] = Translator::translate('error_backup_create_failed');
        header('Location: /beheer/?tab=backup');
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore_backup'])) {
    if (!verify_csrf()) {
        ...
        $_SESSION['flash_error'] = Translator::translate('error_upload_failed');
    } else {
        $file = $_FILES['backup_file'];
        $zip = new ZipArchive();
        if ($zip->open($file['tmp_name']) !== true) {
            $_SESSION['flash_error'] = Translator::translate('error_invalid_zip');
        } else {
            ...
            if (!$valid) {
                $_SESSION['flash_error'] = Translator::translate('error_invalid_backup_no_data');
            } else {
...
echo Translator::translate('backup_title');
?></h1>
...
echo Translator::translate('backup_download_heading');
?></h2>
        <p class="text-sm text-gray-400 mb-4"><?php 
echo Translator::translate('backup_download_desc');
?></p>
...
?> <?php 
echo Translator::translate('backup_data_files_count');
?></p>
            <p><?php 
echo count($subFiles);
?> <?php 
echo Translator::translate('backup_form_submissions_count');
?></p>
            <p><?php 
echo Translator::translate('backup_total_size');
?> ~<?php 
echo round($totalSize / 1024, 1);
?> KB</p>
        </div>

        <a href="/beheer/?tab=backup&action=download&csrf_token=<?php 
echo csrf_token();
?>" class="btn-admin btn-admin-primary">
            <?php 
echo Translator::translate('backup_download_button');
?>
        </a>
        <p class="text-xs text-gray-500 mt-3"><span class="help-tooltip" data-help="<?php 
echo Translator::translate('tooltip_backup_download');
?>">?</span> <?php 
echo Translator::translate('backup_tip');
?></p>
    </div>
    ...
    <div class="admin-card">
        <h2 class="text-lg font-semibold text-white mb-4"><?php 
echo Translator::translate('backup_restore_heading');
?></h2>
        <p class="text-sm text-gray-400 mb-4"><?php 
echo Translator::translate('backup_restore_desc');
?> <span class="help-tooltip" data-help="<?php 
echo Translator::translate('tooltip_backup_restore');
?>">?</span></p>

        <form method="POST" enctype="multipart/form-data" onsubmit="return confirm('<?php 
echo Translator::translate('confirm_restore_backup');
?>')">
            <?php 
echo csrf_field();
?>
            <input type="file" name="backup_file" accept=".zip" required class="admin-input w-full mb-3">
            <button type="submit" name="restore_backup" class="btn-admin btn-admin-danger"><?php 
echo Translator::translate('backup_restore_button');
?></button>
        </form>
    </div>
</div>
