...
<h1 class="text-2xl font-bold text-white mb-6"><?= t('email_settings_title') ?></h1>

<!-- SMTP Settings -->
<form method="POST" class="admin-card mb-6">
    <?= csrf_field() ?>

    <h3 class="text-md font-semibold text-white mb-4"><?= t('smtp_config_heading') ?></h3>
    <p class="text-sm text-gray-400 mb-6"><?= t('smtp_config_desc') ?></p>

    <div class="mb-4">
        <label class="flex items-center gap-2 cursor-pointer">
            <input type="hidden" name="smtp_enabled" value="0">
            <input type="checkbox" name="smtp_enabled" value="1" <?= !empty($smtp['enabled']) ? 'checked' : '' ?> class="w-4 h-4 rounded">
            <span class="text-sm text-gray-300"><?= t('smtp_enable_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_enable') ?>">?</span></span>
        </label>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_server_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_server') ?>">?</span></label>
            <input type="text" name="smtp_host" value="<?= e($smtp['host'] ?? '') ?>" class="admin-input w-full" placeholder="smtp.hostinger.com">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_port_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_port') ?>">?</span></label>
            <select name="smtp_port" class="admin-input w-full">
                ...
                <option value="25" <?= ($smtp['port'] ?? '') === '25' ? 'selected' : '' ?>><?= t('smtp_port_25_label') ?></option>
            </select>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_encryption_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_encryption') ?>">?</span></label>
            <select name="smtp_encryption" class="admin-input w-full">
                ...
                <option value="none" <?= ($smtp['encryption'] ?? '') === 'none' ? 'selected' : '' ?>><?= t('smtp_encryption_none') ?></option>
            </select>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_username_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_username') ?>">?</span></label>
            <input type="text" name="smtp_username" value="<?= e($smtp['username'] ?? '') ?>" class="admin-input w-full" placeholder="noreply@uwdomein.nl">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_password_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_password') ?>">?</span></label>
            <input type="password" name="smtp_password" value="" class="admin-input w-full" placeholder="<?= $hasPassword ? '••••••••' : '' ?>" autocomplete="new-password">
            <?php if ($hasPassword): ?>
            <p class="text-xs text-gray-500 mt-1"><?= t('hint_smtp_password_set') ?></p>
            <?php endif; ?>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_from_email_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_from_email') ?>">?</span></label>
            <input type="email" name="smtp_from_email" value="<?= e($smtp['from_email'] ?? '') ?>" class="admin-input w-full" placeholder="noreply@uwdomein.nl">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_from_name_label') ?> <span class="help-tooltip" data-help="<?= t('tooltip_smtp_from_name') ?>">?</span></label>
    ...
    <div class="mt-6">
        <button type="submit" name="save_smtp" class="btn-admin btn-admin-primary"><?= t('button_save') ?></button>
    </div>
</form>
...
<form method="POST" class="admin-card mb-6">
    ...
    <h3 class="text-md font-semibold text-white mb-4"><?= t('smtp_test_heading') ?></h3>
    <p class="text-sm text-gray-400 mb-4"><?= t('smtp_test_desc') ?></p>

    <div class="flex items-end gap-3">
        <div class="flex-1">
            <label class="block text-sm font-medium text-gray-300 mb-1"><?= t('smtp_test_recipient_label') ?></label>
        ...
        <button type="submit" name="send_test" class="btn-admin btn-admin-outline whitespace-nowrap"><?= t('smtp_test_send_button') ?></button>
    </div>
</form>

<!-- Provider configs -->
<div class="admin-card">
    <h3 class="text-md font-semibold text-white mb-4"><?= t('smtp_common_configs_heading') ?></h3>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-gray-800/50 rounded-lg p-4">
            <h4 class="text-sm font-semibold text-white mb-2">Hostinger</h4>
            <div class="text-xs text-gray-400 space-y-1">
                <p><?= t('smtp_config_server_label') ?> <span class="text-gray-300">smtp.hostinger.com</span></p>
                <p><?= t('smtp_config_port_label') ?> <span class="text-gray-300">465</span></p>
                <p><?= t('smtp_config_encryption_label') ?> <span class="text-gray-300">SSL</span></p>
                <p><?= t('smtp_config_username_label') ?> <span class="text-gray-300"><?= t('smtp_hostinger_username_hint') ?></span></p>
            </div>
        </div>
        ...
        <div class="bg-gray-800/50 rounded-lg p-4">
            ...
            <div class="text-xs text-gray-400 space-y-1">
                <p><?= t('smtp_config_server_label') ?> <span class="text-gray-300">smtp.gmail.com</span></p>
                <p><?= t('smtp_config_port_label') ?> <span class="text-gray-300">587</span></p>
                <p><?= t('smtp_config_encryption_label') ?> <span class="text-gray-300">TLS</span></p>
                <p><?= t('smtp_config_username_label') ?> <span class="text-gray-300"><?= t('smtp_gmail_username_hint') ?></span></p>
                <p class="text-yellow-500 mt-1"><?= t('smtp_gmail_app_password_warning') ?></p>
            </div>
        </div>
        ...
        <div class="bg-gray-800/50 rounded-lg p-4">
            ...
            <div class="text-xs text-gray-400 space-y-1">
                <p><?= t('smtp_config_server_label') ?> <span class="text-gray-300">smtp.transip.email</span></p>
                <p><?= t('smtp_config_port_label') ?> <span class="text-gray-300">465</span></p>
                <p><?= t('smtp_config_encryption_label') ?> <span class="text-gray-300">SSL</span></p>
            </div>
        </div>
