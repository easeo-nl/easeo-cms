...
<section class="py-12">
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <!-- Form -->
            <div>
                <h1 class="text-3xl font-display font-bold text-dark mb-4"><?php 
echo ContentRepository::escape($contactData['titel'] ?? 'Contact');
?></h1>
                <?php 
if (!empty($contactData['intro_tekst'])) {
    ?>
                <p class="text-muted mb-6"><?php 
    echo ContentRepository::escape($contactData['intro_tekst']);
    ?></p>
                <?php 
}
?>
...
if (ContentRepository::siteValue('company.address')) {
    ?>
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="font-medium text-dark"><?php 
...
if (ContentRepository::siteValue('company.email')) {
    ...
    ?>" class="text-primary hover:underline"><?php 
    echo ContentRepository::escape(ContentRepository::siteValue('company.email'));
    ?></a>
                        </div>
                        <?php 
...
?>
                        ...
                        <?php 
if (ContentRepository::siteValue('company.kvk')) {
    ?>
...
?>
                ...
                <?php 
if (!empty($contactData['kaart_embed'])) {
    ?>
                <div class="mt-6 rounded-lg overflow-hidden">
                    <?php 
    echo $contactData['kaart_embed'];
    ?>
                </div>
                <?php 
}
